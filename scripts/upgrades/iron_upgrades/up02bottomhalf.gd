class_name Upgrade02GeneratorUpgrade
extends Upgrade
## Upgrade 01 - Increases gold each Second

## Load level data
func _init() -> void:
	level = Game.ref.data.up_02_levelG
	title = "Iron Leggings and Boots"
	base_cost = 5
	calculate_cost()

func description() -> String:
	var description : String = "Increases Iron obtained each second."
	description += "\nEffects : +1 Iron per Second / Level"
	description += "\nCost : %s" %current_cost
...
func level_up() -> void:
	...
	if not error: 
		level += 1
		Game.ref.data.up_02_levelG = level
